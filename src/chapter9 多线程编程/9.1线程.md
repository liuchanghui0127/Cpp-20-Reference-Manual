...
使用<thread>中定义的线程库可以进行线程的创建，同时可以使用多种方式指定线程需要执行的内容

### **9.1.1通过函数指针创建线程**

thread创建的线程使用的函数可以有任意数量的参数，第一个参数为要执行的函数名称，后续参数为要传入函数的变量，示例如下所示：
```
import <iostream>;
import <thread>;
using namespace std;
...
```
注意，在销毁线程前需要调用join()或detach()方法，使得线程变为不可结合，否则会调用terminate()终止所有线程和程序。
